...
# i think there's a 96 input missing
...
# change values of pps-typed-in answers, like poland and Poland
#...
...
write.csv2(logbook_toExp, paste0(tabDir, ".csv"), row.names=FALSE)
```
...
write.csv2(logbook_df, paste0(tabDir, "logbook_BRAC2_Pro_27-28-3.csv"), row.names=FALSE)
```
...
# add demogrpahics variables to the dataset
for (j in unique(dclean$pp)){
  dclean[dclean$pp == j, "age"] <- demo[demo$pp == j, "age"]
  dclean[dclean$pp == j, "sex"]  <- demo[demo$pp == j, "sex"]
  dclean[dclean$pp == j, "psyKnowledge"]  <- demo[demo$pp == j, "psyKnowledge"]
  dclean[dclean$pp == j, "education"] <- demo[demo$pp == j, "education"]
  dclean[dclean$pp == j, "handedness"]  <- demo[demo$pp == j, "handedness"]
  dclean[dclean$pp == j, "motherTongue"]  <- demo[demo$pp == j, "motherTongue"]
  dclean[dclean$pp == j, "country"] <- demo[demo$pp == j, "country"]
}
...
for (p in c("GM4", "LU1", "LY8", "NE3", "NW8", "TO3", "UG7", "WI5", "XL2", "YF1", "5896f9951a06b50001a7ff12", "5b1fc3ae325d600001c03dcb", "5d5bcd6c0802ee001608c5a8", "5e4a932344498f4e361fee0d", "5ec3dd4a21dd590667039a02")){
  dclean[dclean$pp == p, "counterbalance"] <- "horiAA_1st300"
}

# save the dataset ready for the analyses
write.csv2(dclean, paste0(dataDir, "B2_RWTH1.csv"), row.names=FALSE)
```
