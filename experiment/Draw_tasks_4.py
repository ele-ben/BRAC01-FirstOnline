...
for task in tasks:
    for key in keys:
        for key1 in keys1:
            instr = pd.DataFrame([], columns = ["display", "horiMap", "verMap", "greatM", "lessM", "oddM", "evenM"])
            row = {"display": "Instructions", "horiMap": horiMap + task[0],
                "verMap": verMap + task[1], "greatM": key[0] + greatM, "lessM": key[1]+ lessM, "oddM": key1[0] + oddM, "evenM": key1[1] + evenM}
