...
emm_options(pbkrtest.limit = 27871)
...
#write.table(d, paste0(dataDir, "temp_B1_B2_ageEstimated.csv"), sep = ";", dec = ".", row.names=FALSE)
```
...
d1 <- drt[(drt$pp == "NY3" | drt$pp == "VK1") & (drt$context_R == 1),]

library(nlme)
Oats.lmer <- lme4::lmer(yield ~ Variety + factor(nitro) + (1|Block/Variety),
                        data = nlme::Oats, subset = -c(1,2,3,5,8,13,21,34,55))

Oats.emm.n <- emmeans(Oats.lmer, c("nitro", "Variety"))

install.packages("lsmeans")
library(lsmeans)
LSmeans(Oats.lmer, effect = "nitro")
```
...
if (B == "B1"){
  ...
  mod1 <- lmer(rt ~ task_R*ANSWER_R*context_R*cocoa + (1|pp), data= drt, REML=T)
}



vcovmod1 <- vcov.merMod(mod1)
summary(mod1)

ciao <- emmeans(mod1, c("task_R", "ANSWER_R", "context_R", "cocoa"), lmer.df = "satterthwaite")
ref_grid(mod1, data = drt ) @ grid
ref_grid(mod1bis, data = drt ) @ grid
emmeans(mod1) @ grid

get_emm_option("lmer.df")


mod1bis <- lmer(logRT ~ task_R + ANSWER_R + (1|pp), data= drt, REML=T)
...
if (B == "B1" | B == "B2"){
  ...
  mod2 <- lmer(logRT ~ task_R*ANSWER_R*context_R*cocoa + blockNum + age + sex + map_horiAA300 +
    ...
    # block num doens't interact signifcantly with ANS*task - so no evidence they learnt about different
    # frequency of resp sw or rep
